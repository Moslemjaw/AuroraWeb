...
export default function Cart() {
  const { items, updateQuantity, removeFromCart, getTotalFormatted, itemCount } = useCart();
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-12 lg:px-24 py-6 sm:py-12">
        ...
        {items.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-8">Looks like you haven't added any beautiful blooms yet.</p>
          ...
          <div className="grid lg:grid-cols-3 gap-8 sm:gap-12">
            ...
            <div className="bg-secondary/10 p-5 sm:p-8 h-fit rounded-sm sticky top-28">
              ...
              <div className="flex justify-between items-end mb-6 sm:mb-8">
                ...
                <span className="font-serif text-xl sm:text-2xl text-primary">{getTotalFormatted()}</span>
              </div>
              <Button asChild className="w-full h-11 sm:h-12 bg-primary hover:bg-primary/90 text-white rounded-none uppercase tracking-widest text-[10px] sm:text-xs font-bold">
                <Link href="/checkout" data-testid="button-checkout">Proceed to Checkout</Link>
              </Button>
            </div>
          </div>
        )}
